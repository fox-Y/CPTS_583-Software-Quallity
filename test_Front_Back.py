#Tianxiang: responsible for the test of database
#Modify the administrators' login from read userinfo from .txt file
#to query and compare user data from database
#All data query were from databasae
#Riley: responsible for the test for the login security
#Test the software security by hash table
...
class Application(Frame):
    ...
    def createWidget(self):
        ...
        self.db = m.connect(host="localhost", user="root", passwd="Xtx123456", db="DB_Voting")
    ...
    def buttonClicklogin(self, master):
        self.res = dataManage.adLogin(self.euser.get(),self.epass.get())
        print(self.res)
        self.flag = 0

        if (self.res[0] == self.euser.get() and self.res[1] == self.epass.get()):
            self.flag = 1
